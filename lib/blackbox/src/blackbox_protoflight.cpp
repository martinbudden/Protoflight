...
*/
Blackbox::write_e BlackboxProtoflight::write_system_information(const blackbox_context_t& ctx)
{
    ...
    if (!header_reserve_buffer_space()) {
...
// See https://github.com/betaflight/blackbox-log-viewer/blob/master/src/flightlog_parser.js for parsing of fields
    ...
    const dynamic_idle_controller_config_t* dynamicIdleControllerConfig = ctx.motor_mixer.get_dynamic_idle_config();
#endif
...
//        BLACKBOX_PRINT_HEADER_LINE("P denom", "%d",                         static_cast<uint16_t>(blackboxIInterval / blackboxPInterval));
        ...
        BLACKBOX_PRINT_HEADER_LINE("maxthrottle", "%d",                     ctx.motor_mixer.get_motor_config().max_throttle);
// Baseflight uses a GYRO_SCALE that gives radians per microsecond as output, whereas Cleanflight produces degrees
...
*/
        ...
        BLACKBOX_PRINT_HEADER_LINE("looptime", "%d",                        ctx.flight_controller.get_task_interval_microseconds());
        ...
        BLACKBOX_PRINT_HEADER_LINE("thr_mid", "%d",                         rates.throttle_midpoint);
        BLACKBOX_PRINT_HEADER_LINE("thr_expo", "%d",                        rates.throttle_expo);
...
//H velPID:55,55,75
*/
        BLACKBOX_PRINT_HEADER_LINE("rc_rates", "%d,%d,%d",                  rates.rc_rates[rates_t::ROLL],
                                                                            rates.rc_rates[rates_t::PITCH],
                                                                            rates.rc_rates[rates_t::YAW]);
        BLACKBOX_PRINT_HEADER_LINE("rc_expo", "%d,%d,%d",                   rates.rc_expos[rates_t::ROLL],
                                                                            rates.rc_expos[rates_t::PITCH],
                                                                            rates.rc_expos[rates_t::YAW]);
        BLACKBOX_PRINT_HEADER_LINE("rates", "%d,%d,%d",                     rates.rates[rates_t::ROLL],
                                                                            rates.rates[rates_t::PITCH],
                                                                            rates.rates[rates_t::YAW]);
        BLACKBOX_PRINT_HEADER_LINE("rate_limits", "%d,%d,%d",               rates.rate_limits[rates_t::ROLL],
                                                                            rates.rate_limits[rates_t::PITCH],
                                                                            rates.rate_limits[rates_t::YAW]);
        BLACKBOX_PRINT_HEADER_LINE("rollPID", "%d,%d,%d",                   ctx.flight_controller.get_pid_msp(FlightController::ROLL_RATE_DPS).kp,
                                                                            ctx.flight_controller.get_pid_msp(FlightController::ROLL_RATE_DPS).ki,
                                                                            ctx.flight_controller.get_pid_msp(FlightController::ROLL_RATE_DPS).kd);
        BLACKBOX_PRINT_HEADER_LINE("pitchPID", "%d,%d,%d",                  ctx.flight_controller.get_pid_msp(FlightController::PITCH_RATE_DPS).kp,
                                                                            ctx.flight_controller.get_pid_msp(FlightController::PITCH_RATE_DPS).ki,
                                                                            ctx.flight_controller.get_pid_msp(FlightController::PITCH_RATE_DPS).kd);
        BLACKBOX_PRINT_HEADER_LINE("yawPID", "%d,%d,%d",                    ctx.flight_controller.get_pid_msp(FlightController::YAW_RATE_DPS).kp,
                                                                            ctx.flight_controller.get_pid_msp(FlightController::YAW_RATE_DPS).ki,
                                                                            ctx.flight_controller.get_pid_msp(FlightController::YAW_RATE_DPS).kd);
        BLACKBOX_PRINT_HEADER_LINE("rollAnglePID", "%d,%d,%d",              ctx.flight_controller.get_pid_msp(FlightController::ROLL_ANGLE_DEGREES).kp,
                                                                            ctx.flight_controller.get_pid_msp(FlightController::ROLL_ANGLE_DEGREES).ki,
                                                                            ctx.flight_controller.get_pid_msp(FlightController::ROLL_ANGLE_DEGREES).kd);
        BLACKBOX_PRINT_HEADER_LINE("pitchAnglePID", "%d,%d,%d",             ctx.flight_controller.get_pid_msp(FlightController::PITCH_ANGLE_DEGREES).kp,
                                                                            ctx.flight_controller.get_pid_msp(FlightController::PITCH_ANGLE_DEGREES).ki,
                                                                            ctx.flight_controller.get_pid_msp(FlightController::PITCH_ANGLE_DEGREES).kd);
/*
...
*/
        BLACKBOX_PRINT_HEADER_LINE("dterm_lpf1_static_hz", "%d",            ctx.flight_controller.get_filters_config().dterm_lpf1_hz);
        BLACKBOX_PRINT_HEADER_LINE("dterm_lpf2_static_hz", "%d",            ctx.flight_controller.get_filters_config().dterm_lpf2_hz);
#if defined(USE_DTERM_FILTERS_EXTENDED)
        BLACKBOX_PRINT_HEADER_LINE("dterm_lpf1_type", "%d",                 ctx.flight_controller.get_filters_config().dterm_lpf1_type);
        BLACKBOX_PRINT_HEADER_LINE("dterm_lpf2_type", "%d",                 ctx.flight_controller.get_filters_config().dterm_lpf2_type);
        //BLACKBOX_PRINT_HEADER_LINE("yaw_lowpass_hz", "%d",                  current_pid_profile.yaw_lowpass_hz);
        BLACKBOX_PRINT_HEADER_LINE("dterm_notch_hz", "%d",                  ctx.flight_controller.get_filters_config().dterm_notch_hz);
        BLACKBOX_PRINT_HEADER_LINE("dterm_notch_cutoff", "%d",              ctx.flight_controller.get_filters_config().dterm_notch_cutoff);
#endif
        //BLACKBOX_PRINT_HEADER_LINE("iterm_windup", "%d",                    current_pid_profile.itermWindup);
        //BLACKBOX_PRINT_HEADER_LINE("pid_at_min_throttle", "%d",             current_pid_profile.pidAtMinThrottle);

        // Betaflight PID controller parameters
        BLACKBOX_PRINT_HEADER_LINE("anti_gravity_gain", "%d",               ctx.flight_controller.get_anti_gravity_config().i_gain);
        BLACKBOX_PRINT_HEADER_LINE("anti_gravity_cutoff_hz", "%d",          ctx.flight_controller.get_anti_gravity_config().cutoff_hz);
        BLACKBOX_PRINT_HEADER_LINE("anti_gravity_p_gain", "%d",             ctx.flight_controller.get_anti_gravity_config().p_gain);
#if defined(USE_INTEGRATED_YAW_CONTROL)
        BLACKBOX_PRINT_HEADER_LINE("use_integrated_yaw", "%d",              current_pid_profile.use_integrated_yaw);
#endif
        BLACKBOX_PRINT_HEADER_LINE("s_roll", "%d",                          ctx.flight_controller.get_pid_msp(FlightController::ROLL_RATE_DPS).ks)
        BLACKBOX_PRINT_HEADER_LINE("s_pitch", "%d",                         ctx.flight_controller.get_pid_msp(FlightController::PITCH_RATE_DPS).ks)
        BLACKBOX_PRINT_HEADER_LINE("s_yaw", "%d",                           ctx.flight_controller.get_pid_msp(FlightController::YAW_RATE_DPS).ks)
        BLACKBOX_PRINT_HEADER_LINE("ff_weight", "%d,%d,%d",                 ctx.flight_controller.get_pid_msp(FlightController::ROLL_RATE_DPS).kk,
                                                                            ctx.flight_controller.get_pid_msp(FlightController::PITCH_RATE_DPS).kk,
                                                                            ctx.flight_controller.get_pid_msp(FlightController::YAW_RATE_DPS).kk);
...
*/
        BLACKBOX_PRINT_HEADER_LINE("gyro_lpf1_type", "%d",                  ctx.imu_filters.get_config().gyro_lpf1_type);
        BLACKBOX_PRINT_HEADER_LINE("gyro_lpf1_static_hz", "%d",             ctx.imu_filters.get_config().gyro_lpf1_hz);
        BLACKBOX_PRINT_HEADER_LINE("gyro_lpf2_type", "%d",                  ctx.imu_filters.get_config().gyro_lpf2_type);
        BLACKBOX_PRINT_HEADER_LINE("gyro_lpf2_static_hz", "%d",             ctx.imu_filters.get_config().gyro_lpf2_hz);
...
*/
        BLACKBOX_PRINT_HEADER_LINE("acc_lpf_hz", "%d",                      ctx.imu_filters.get_config().acc_lpf_hz);
        ...
        BLACKBOX_PRINT_HEADER_LINE("motor_idle", "%d",                      ctx.motor_mixer.get_motor_config().motor_idle);
        BLACKBOX_PRINT_HEADER_LINE("debug_mode", "%d",                      get_debug_mode());
