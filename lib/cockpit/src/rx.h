...
struct rx_config_t {
    ...
    uint8_t fpvCam_angle_degrees;         // Camera angle to be scaled into rc commands
...
struct rx_channel_mode_t {
    uint8_t mode; // failsafe_channel_mode_e
    uint8_t step;
};

struct rx_channel_range_t {
    uint16_t min;
    uint16_t max;
};

static constexpr uint8_t RX_MAX_SUPPORTED_RC_CHANNEL_COUNT = 18;
static constexpr uint8_t RX_STICK_CHANNEL_COUNT = 4;
static constexpr uint8_t RX_MAX_SUPPORTED_RC_PARALLEL_PWM_CHANNEL_COUNT = 8;
static constexpr uint8_t RX_MAX_AUX_CHANNEL_COUNT = RX_MAX_SUPPORTED_RC_CHANNEL_COUNT - RX_STICK_CHANNEL_COUNT;

typedef std::array<rx_channel_mode_t, RX_MAX_SUPPORTED_RC_CHANNEL_COUNT> rx_failsafe_channel_configs_t;
typedef std::array<rx_channel_range_t, RX_STICK_CHANNEL_COUNT> rx_channel_range_configs_t;
...
public:
    rx_config_t _config;
    rx_failsafe_channel_configs_t _failsafeChannelConfigs;
    rx_channel_range_configs_t _channelRangeConfigs;
};
