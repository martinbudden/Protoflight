...
struct autopilot_config_t {
    pid_constants_uint16_t altitude_pid;
    pid_constants_uint16_t position_pid;
    uint16_t landing_altitude_meters;
...
public:
    ...
    Autopilot(const AhrsMessageQueue& ahrs_message_queue) : _ahrs_message_queue(ahrs_message_queue) {}
    Autopilot(const AhrsMessageQueue& ahrs_message_queue, AltitudeMessageQueue& altitudeMessageQueue) : _ahrs_message_queue(ahrs_message_queue), _altitudeMessageQueue(&altitudeMessageQueue) {}
private:
...
public:
    ...
    fc_controls_t calculateFlightControls(const receiver_controls_t& controls, uint32_t flightMode_mode_flags);

    AltitudeMessageQueue* getAltitudeMessageQueue_mutable() { return _altitudeMessageQueue; }
    const AltitudeMessageQueue* getAltitudeMessageQueue() const { return _altitudeMessageQueue; }
private:
    const AhrsMessageQueue& _ahrs_message_queue;
