...
void Main::calibrate_imu_and_save(NonVolatileStorage& nvs, ImuBase& imu, calibration_type_e calibration_type)
{
#if defined(M5_UNIFIED)
    if (M5.Lcd.width() > 300) {
    ...
    imu.calibrate(calibration_type == CALIBRATE_ACC_AND_GYRO ? ImuBase::CALIBRATE_ACC_AND_GYRO : ImuBase::CALIBRATE_GYRO_ONLY, CALIBRATION_COUNT);
...
void Main::check_imu_calibration(NonVolatileStorage& nvs, ImuBase& imu) // cppcheck-suppress constParameterReference
{
    ...
    if (nvs.load_gyro_calibration_state() == NonVolatileStorage::CALIBRATED) {
        ...
        //!!calibrate_imu_and_save(nvs, imu, CALIBRATE_GYRO_ONLY);
    }
}
