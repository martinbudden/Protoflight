...
*/
ImuBase& Main::create_imu(NonVolatileStorage& nvs)
{
    ...
    if (M5.BtnA.isPressed()) {
        calibrate_imu_and_save(nvs, imuSensor, CALIBRATE_ACC_AND_GYRO);
    }
#endif
    check_imu_calibration(nvs, imuSensor);

    const uint32_t imuSample_rate_hz = imuSensor.get_gyro_sample_rate_hz();
    std::array<char, 128> buf;
    sprintf(&buf[0], "\r\n**** IMU sample rate:%dHz\r\n\r\n", static_cast<int>(imuSample_rate_hz));
    print(&buf[0]);
