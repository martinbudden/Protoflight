...
ReceiverBase& Main::create_receiver(NonVolatileStorage& nvs)
{
    (void)nvs;
#if defined(LIBRARY_RECEIVER_USE_ESPNOW)
    ...
    static SerialPort serial_port(SerialPort::RECEIVER_PINS, RECEIVER_UART_INDEX, ReceiverSbus::BAUD_RATE, ReceiverSbus::DATA_BITS, ReceiverSbus::STOP_BITS, ReceiverSbus::PARITY);
    static ReceiverSbus receiver(serial_port);
    return receiver;

#elif defined(USE_RECEIVER_IBUS)

    static SerialPort serial_port(SerialPort::RECEIVER_PINS, RECEIVER_UART_INDEX, ReceiverIbus::BAUD_RATE, ReceiverIbus::DATA_BITS, ReceiverIbus::STOP_BITS, ReceiverIbus::PARITY);
    static ReceiverIbus receiver(serial_port);
    return receiver;

#elif defined(USE_RECEIVER_CRSF)

    static SerialPort serial_port(SerialPort::RECEIVER_PINS, RECEIVER_UART_INDEX, ReceiverCrsf::BAUD_RATE, ReceiverCrsf::DATA_BITS, ReceiverCrsf::STOP_BITS, ReceiverCrsf::PARITY);
    static ReceiverCrsf receiver(serial_port);
    return receiver;

#else

    const rx_config_t& rx_config = nvs.load_rx_config();
    auto rxType = static_cast<RX::serial_type_e>(rx_config.serial_rx_provider);
    ...
    static SerialPort serial_port(SerialPort::RECEIVER_PINS, RECEIVER_UART_INDEX, baudrate, dataBits, stopBits, parity);
    ...
    switch (rxType) {
    case RX::SERIAL_SBUS:
        receiverPtr = new(&receiverMemory[0]) ReceiverSbus(serial_port);
        break;
    case RX::SERIAL_IBUS:
        receiverPtr = new(&receiverMemory[0]) ReceiverIbus(serial_port);
        break;
    case RX::SERIAL_CRSF:
        receiverPtr = new(&receiverMemory[0]) ReceiverCrsf(serial_port);
        break;
    default:
