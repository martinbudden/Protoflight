...
void ESC_DShotBitbang::outputToMotors(uint16_t m1, uint16_t m2, uint16_t m3, uint16_t m4)
{
    ...
    DMA2_Stream6->M0AR = reinterpret_cast<uint32_t>(&dma_output_buffer_1_4[0]);
    ...
    DMA2_Stream2->M0AR = reinterpret_cast<uint32_t>(&dma_output_buffer_2_3[0]);
...
void ESC_DShotBitbang::setDMA_outputBuffers(uint16_t m1_value, uint16_t m2_value, uint16_t m3_value, uint16_t m4_value)
{
#if defined(BIT_BANGING_V2)
    setDMA_outputBuffersV2(m1_value, m2_value, m3_value, m4_value);
#else
    setDMA_outputBuffersV1(m1_value, m2_value, m3_value, m4_value);
#endif
}
...
We can preset the DMA output buffer because whether the output bit is 0 or 1, the first two sections are zero and the last section is one
DSHOT_BB_FRAME_SECTIONS = 14
DSHOT_BB_0_LENGTH = 10
DSHOT_BB_1_LENGTH = 4
0 bit = 00001111111111
1 bit = 00000000001111

Note bidirectional DShot is inverted, corresponding outputs for unidirectional DShot are:
0 bit = 11110000000000
...
void ESC_DShotBitbang::presetDMA_outputBuffersV1()
{
    ...
    dma_output_buffer_1_4.fill(0);
    dma_output_buffer_2_3.fill(0);
    ...
    for (size_t i = 0; i < DSHOT_BB_BUFFER_LENGTH - 2; i++) {
        ...
        // GPIO_BSRR_BR_0 means set low
        dma_output_buffer_1_4[index] = (GPIO_BSRR_BR_0 << MOTOR_1) | (GPIO_BSRR_BR_0 << MOTOR_4); // NOLINT(bugprone-implicit-widening-of-multiplication-result)
        dma_output_buffer_2_3[index] = (GPIO_BSRR_BR_0 << MOTOR_2) | (GPIO_BSRR_BR_0 << MOTOR_3); // NOLINT(bugprone-implicit-widening-of-multiplication-result)
        // set transition to high after DSHOT_BB_1_LENGTH length:
        dma_output_buffer_1_4[index + DSHOT_BB_1_LENGTH - 1] = (GPIO_BSRR_BS_0 << MOTOR_1) | (GPIO_BSRR_BS_0 << MOTOR_4);
        dma_output_buffer_2_3[index + DSHOT_BB_1_LENGTH - 1] = (GPIO_BSRR_BS_0 << MOTOR_2) | (GPIO_BSRR_BS_0 << MOTOR_3);
    }
}
...
void ESC_DShotBitbang::setDMA_outputBuffersV1(uint16_t m1_value, uint16_t m2_value, uint16_t m3_value, uint16_t m4_value)
{
    ...
    for (size_t i = 0; i < DSHOT_BB_BUFFER_LENGTH - 2; i++) { // last 2 bits are always high (logic 0)
        // get the index of the transition bit
        const size_t index  = i * DSHOT_BB_FRAME_SECTIONS + DSHOT_BB_0_LENGTH - 1;

        const uint32_t motorMask = 1 << (DSHOT_BB_BUFFER_LENGTH - 3 - i);
        // note: bidirectional DShot is inverted
        // if bit is ONE send 0x00 so that GPIOs output will not change (will stay LOW):
        // if bit is ZERO set transition to HIGH after DSHOT_BB_0_LENGTH:
        // GPIO_BSRR_BS_0 means transition to HIGH, 0x00 means stay the same
        dma_output_buffer_1_4[index] =  (motorMask & m1_value) ? 0x00 : GPIO_BSRR_BS_0 << MOTOR_1;
        dma_output_buffer_1_4[index] |= (motorMask & m4_value) ? 0x00 : GPIO_BSRR_BS_0 << MOTOR_4;
        dma_output_buffer_2_3[index] =  (motorMask & m2_value) ? 0x00 : GPIO_BSRR_BS_0 << MOTOR_2;
        dma_output_buffer_2_3[index] |= (motorMask & m3_value) ? 0x00 : GPIO_BSRR_BS_0 << MOTOR_3;
    }
}
...
void ESC_DShotBitbang::presetDMA_outputBuffersV2()
{
    ...
    for (auto i = 0; i < (DSHOT_BB_BUFFER_LENGTH - 2); i++) { // last 2 bits are always high (logic 0)
        ...
        dma_output_buffer_1_4[i * DSHOT_BB_FRAME_SECTIONS + 2] = GPIO_BSRR_BS_0 << MOTOR_1 | GPIO_BSRR_BS_0 << MOTOR_4;
        dma_output_buffer_2_3[i * DSHOT_BB_FRAME_SECTIONS + 2] = GPIO_BSRR_BS_0 << MOTOR_2 | GPIO_BSRR_BS_0 << MOTOR_3;
    }
}
