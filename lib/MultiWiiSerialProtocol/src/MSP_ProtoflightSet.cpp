...
msp_result_e MSP_Protoflight::process_set_command(msp_parameter_group_t& pg, int16_t cmd_msp, StreamBufReader& src) // NOLINT(readability-function-cognitive-complexity)
{
    ...
    case MSP_SET_PID: {
        ...
        for (size_t ii = 0; ii < FlightController::RPY_AXIS_COUNT; ++ii) {
            ...
            pg.flightController.set_pid_p_msp(pid_index, src.read_u8());
            pg.flightController.set_pid_i_msp(pid_index, src.read_u8());
            pg.flightController.set_pid_d_msp(pid_index, src.read_u8());
        }
        const uint8_t kp = src.read_u8();
        const uint8_t ki = src.read_u8();
        const uint8_t kd = src.read_u8();
        pg.flightController.set_pid_p_msp(FlightController::ROLL_ANGLE_DEGREES, kp);
        pg.flightController.set_pid_i_msp(FlightController::ROLL_ANGLE_DEGREES, ki);
        pg.flightController.set_pid_d_msp(FlightController::ROLL_ANGLE_DEGREES, kd);
        pg.flightController.set_pid_p_msp(FlightController::PITCH_ANGLE_DEGREES, kp);
        pg.flightController.set_pid_i_msp(FlightController::PITCH_ANGLE_DEGREES, ki);
        pg.flightController.set_pid_d_msp(FlightController::PITCH_ANGLE_DEGREES, kd);
        // skip over PID_MAG
        src.read_u8();
        src.read_u8();
        src.read_u8();
    ...
    case MSP_SET_MODE_RANGE: {
        ...
        rc_modes_activation_condition_t mac = pg.rc_modes.get_mode_activation_condition(macIndex);
        mac.mode_id = box->id;
        ...
        pg.rc_modes.set_mode_activation_condition(macIndex, mac);
        pg.rc_modes.analyze_mode_activation_conditions();
        break;
    }
    case MSP_SET_ADJUSTMENT_RANGE: {
#if defined(USE_RC_ADJUSTMENTS)
        const uint8_t adjustmentRangeIndex = src.read_u8();
        if (adjustmentRangeIndex >= RC_MAX_ADJUSTMENT_RANGE_COUNT) {
            return MSP_RESULT_ERROR;
        }
        RC_Adjustments& rcAdjustments = pg.cockpit.getRC_Adjustments();
        rc_adjustment_range_t adjustmentRange = rcAdjustments.getAdjustmentRange(adjustmentRangeIndex);
        src.read_u8(); // was adjustmentRange.adjustmentIndex
        adjustmentRange.aux_channel_index = src.read_u8();
        adjustmentRange.range_start = src.read_u8();
        adjustmentRange.range_end = src.read_u8();
    ...
    case MSP_SET_RC_TUNING: {
        ...
        rates_t rates = pg.cockpit.getRates();
    ...
    case MSP_SET_GPS_CONFIG: {
        ...
        gps_config_t gpsConfig = pg.gps->getConfig();
    ...
    case MSP_SET_ADVANCED_CONFIG: {
        motor_config_t motorConfig = pg.motorMixer.get_motor_config();
        ...
        if (src.bytes_remaining() >= 1) {
            ...
            pg.debug.setMode(static_cast<debug_mode_e>(src.read_u8()));
        }
        pg.motorMixer.set_motor_config(motorConfig);
        break;
    }
    case MSP_SET_FILTER_CONFIG: {
        imu_filters_config_t imuFiltersConfig = pg.imuFilters.getConfig();
        RpmFilters* rpmFilters = pg.imuFilters.getRPM_FiltersMutable();
        rpm_filters_config_t rpmFiltersConfig = rpmFilters ? rpmFilters->get_config() : rpm_filters_config_t {}; // cppcheck-suppress knownConditionTrueFalse
        flight_controller_filters_config_t fcFilters = pg.flightController.getFiltersConfig();
    ...
    case MSP_SET_PID_ADVANCED: {
        ...
        if (src.bytes_remaining() >= 4) {
            src.read_u16(); // was current_pidProfile->itermThrottleThreshold
            src.read_u16(); // !!TODO: anti_gravity_gain
        }
        if (src.bytes_remaining() >= 2) {
            src.read_u16(); // was current_pidProfile->dtermSetpointWeight
        }
        if (src.bytes_remaining() >= 14) {
            // Added in MSP API 1.40
            src.read_u8(); // !!TODO: iterm_rotation
            src.read_u8(); // was current_pidProfile->smart_feedforward
#if defined(USE_ITERM_RELAX)
            iterm_relax_config_t itermRelaxConfig = pg.flightController.getITermRelaxConfig();
            itermRelaxConfig.iterm_relax = src.read_u8();
            itermRelaxConfig.iterm_relax_type = src.read_u8();
            pg.flightController.setITermRelaxConfig(itermRelaxConfig);
#else
            src.read_u8();
            src.read_u8();
#endif
            ...
            pg.flightController.set_pid_k_msp(FlightController::ROLL_RATE_DPS, src.read_u16());
            pg.flightController.set_pid_k_msp(FlightController::PITCH_RATE_DPS, src.read_u16());
            pg.flightController.set_pid_k_msp(FlightController::YAW_RATE_DPS, src.read_u16());
            src.read_u8(); // was current_pidProfile->antiGravityMode
        }
        break;
    }
    ...
    case MSP_SET_FEATURE_CONFIG:
        pg.cockpit.setFeatures(src.read_u32());
        break;
    ...
    case MSP_SET_MIXER_CONFIG: {
        mixer_config_t mixerConfig = pg.motorMixer.get_mixer_config();
        mixerConfig.type = src.read_u8();
        mixerConfig.yaw_motors_reversed = src.read_u8();
        pg.motorMixer.set_mixer_config(mixerConfig);
        break;
    }
    case MSP_SET_RX_CONFIG: {
        rx_config_t rxConfig = pg.cockpit.getRX_Config();
        ...
        pg.cockpit.setRX_Config(rxConfig);
        break;
    }
    case MSP_SET_FAILSAFE_CONFIG: {
        failsafe_config_t failsafeConfig{};
    ...
    case MSP_SET_RXFAIL_CONFIG: {
        ...
        if (index < RX::MAX_SUPPORTED_RC_CHANNEL_COUNT) {
            RX::failsafe_channel_configs_t rxFailsafeChannelConfigs = pg.cockpit.getRX_FailsafeChannelConfigs();
            ...
            pg.cockpit.setRX_FailsafeChannelConfigs(rxFailsafeChannelConfigs);
            break;
        }
        return MSP_RESULT_ERROR;
    }
    ...
    case MSP_EEPROM_WRITE:
        if (pg.motorMixer.motors_is_on()) {
            // can't save to non volatile storage if the motors are on
            return MSP_RESULT_ERROR;
        }
        pg.nonVolatileStorage.store_all(pg.imuFilters, pg.flightController, pg.motorMixer, pg.cockpit, pg.rc_modes);
        break;
#ifdef USE_BLACKBOX
    case MSP_SET_BLACKBOX_CONFIG:
        // Don't allow config to be updated while Blackbox is logging
        if (pg.blackbox != nullptr && pg.blackbox->mayEditConfig()) {
            Blackbox::config_t blackboxConfig = pg.blackbox->getConfig();
            ...
            if (src.bytes_remaining() >= 2) {
                ...
                pRatio = static_cast<uint16_t>(pg.blackbox->calculateP_Denominator(rateNumerator, rateDenominator));
            }
            ...
            if (src.bytes_remaining() >= 1) {
                ...
                blackboxConfig.sample_rate = static_cast<Blackbox::sample_rate_e>(pg.blackbox->calculateSampleRate(pRatio));
            }
            ...
            pg.blackbox->init(blackboxConfig);
        }
        break;
#endif
    ...
    case MSP_SET_VTX_CONFIG: {
        VTX::type_e vtxType = pg.vtx->getDeviceType();
        vtx_config_t vtxConfig = pg.vtx->getConfig();
        ...
        if (src.bytes_remaining() >= 2) {
            ...
            if (vtxType != VTX::UNKNOWN) {
                ...
                pg.vtx->getStatus(vtxCurrentStatus);
                if ((vtxCurrentStatus & VTX::STATUS_PIT_MODE) != newPitmode) {
                    pg.vtx->setPitMode(newPitmode);
                }
            }
        }
...
void MSP_Protoflight::setMSP_VTX_DeviceStatusReady(msp_parameter_group_t& pg)
{
    (void)pg;
}
