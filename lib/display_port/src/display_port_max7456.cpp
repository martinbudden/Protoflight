...
//Max bytes to update in one call to max7456Draw_screen()
...
{
    _max_screen_size = VIDEO_BUFFER_PAL_CHARACTER_COUNT;
    _small_arrow_up = 0x75;
}
...
{
    return getLayerBuffer(_active_layer);
}
...
void DisplayPortMax7456::setRegisterVM1()
{
    ...
    if (_background_type != BACKGROUND_TRANSPARENT) {
...
void DisplayPortMax7456::reInit()
{
    ...
    case VIDEO_SYSTEM_PAL:
        _videoSignalReg = VIDEO_MODE_PAL | OSD_ENABLE;
        _row_count = VIDEO_LINES_PAL;
        break;
    case VIDEO_SYSTEM_NTSC:
        _videoSignalReg = VIDEO_MODE_NTSC | OSD_ENABLE; // cppcheck-suppress badBitmaskCheck
        _row_count = VIDEO_LINES_NTSC;
        break;
    }
    ...
    if (_videoSignalReg & VIDEO_MODE_PAL) {
        ...
        _max_screen_size = VIDEO_BUFFER_NTSC_CHARACTER_COUNT;
    }
...
DisplayPortMax7456::init_status_e DisplayPortMax7456::init(const config_t* max7456Config, const vcd_profile_t* pVcdProfile, bool cpuOverclock, Debug& debug)
{
    ...
    _background_type = DisplayPortBase::BACKGROUND_TRANSPARENT;
...
uint32_t DisplayPortMax7456::clear_screen(display_clear_option_e options)
{
    (void)options;
    clearLayer(_active_layer);
    return 0;
}

uint32_t DisplayPortMax7456::write_char(uint8_t x, uint8_t y, uint8_t c, uint8_t attr)
{
    (void)attr;
...
uint32_t DisplayPortMax7456::write_string(uint8_t x, uint8_t y, const char *text, uint8_t attr)
{
    (void)attr;
...
bool DisplayPortMax7456::buffersSynced() const
{
    ...
    const auto shadowSpan = std::span(_shadowBuffer).first(_max_screen_size);
    return std::ranges::equal(foregroundSpan, shadowSpan);
#else
    for (size_t ii = 0; ii < _max_screen_size; ++ii) {
...
bool DisplayPortMax7456::reInitIfRequired(bool forceStallCheck, Debug& debug)
{
    ...
    if (stalled) {
        ...
        debug.set(DEBUG_MAX7456_SIGNAL, DEBUG_MAX7456_SIGNAL_ROWS, get_row_count());
...
bool DisplayPortMax7456::draw_screen() // NOLINT(readability-function-cognitive-complexity)
{
    if (!_fontIsLoading) {
        uint8_t *buffer = getActiveLayerBuffer();
        int spi_buf_index = 0;
        ...
        bool autoInc = false;
        int posLimit = _pos + (_max_screen_size / 2);
#if defined(USE_DMA)
        ...
        while ((spi_buf_index < maxSpiBufStartIndex) && (_pos < posLimit) && (time_difference_us(time_us(), startTime) < maxEncodeTime)) { // cppcheck-suppress knownConditionTrueFalse
            if (buffer[_pos] != _shadowBuffer[_pos]) {
                ...
                if (setAddress || !autoInc) {
                    if (buffer[_pos + 1] != _shadowBuffer[_pos + 1]) {
                        ...
                        _spiBuf[spi_buf_index++] = MAX7456ADD_DMM;
                        _spiBuf[spi_buf_index++] = _displayMemoryModeReg | DMM_AUTO_INC;
                        autoInc = true;
                    } else {
                        // It's not worth auto incrementing
                        _spiBuf[spi_buf_index++] = MAX7456ADD_DMM;
                        _spiBuf[spi_buf_index++] = _displayMemoryModeReg;
                        autoInc = false;
                    }
                    _spiBuf[spi_buf_index++] = MAX7456ADD_DMAH;
                    _spiBuf[spi_buf_index++] = static_cast<uint8_t>(_pos >> 8U);
                    _spiBuf[spi_buf_index++] = MAX7456ADD_DMAL;
                    _spiBuf[spi_buf_index++] = _pos & 0xFFU;
                    setAddress = false;
                }
                _spiBuf[spi_buf_index++] = MAX7456ADD_DMDI;
                _spiBuf[spi_buf_index++] = buffer[_pos];
                ...
                if (!setAddress) {
                    ...
                    if (autoInc) {
                        _spiBuf[spi_buf_index++] = MAX7456ADD_DMDI;
                        _spiBuf[spi_buf_index++] = END_STRING;
                    }
                }
            }
            if (++_pos >= _max_screen_size) {
                _pos = 0;
                break;
            }
        }
...
void DisplayPortMax7456::set_background_type(background_e background_type)
{
    _background_type = background_type;
