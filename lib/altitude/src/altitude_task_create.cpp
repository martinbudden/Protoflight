...
AltitudeTask* AltitudeTask::create_task(altitude_context_t& context, uint8_t priority, uint32_t core, uint32_t task_interval_microseconds)
{
    task_info_t task_info {};
    return create_task(task_info, context, priority, core, task_interval_microseconds);
}

AltitudeTask* AltitudeTask::create_task(task_info_t& task_info, altitude_context_t& context, uint8_t priority, uint32_t core, uint32_t task_interval_microseconds)
{
    // Note that task parameters must not be on the stack, since they are used when the task is started, which is after this function returns.
    static AltitudeTask altitude_task(task_interval_microseconds, context);
