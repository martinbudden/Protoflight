...
[[noreturn]] void AltitudeTask::task()
{
    ...
    while (true) {
        ...
        if (_tick_count_delta > 0) { // guard against the case of this while loop executing twice on the same tick interval
            _context.barometer.read_temperature_and_pressure();
            const float altitude_measurement = _context.barometer.calculate_altitude_meters(_context.barometer.get_pressure_pascals(), _context.barometer.get_pressure_pascals());

            ahrs_data_t ahrs_data {};
            _context.ahrs_message_queue.PEEK_AHRS_DATA(ahrs_data);
            //!!TODO: calculate vertical component of acceleration corrected for orientation
            const float acceleration_measurement = ahrs_data.acc_gyro_rps.acc.z;
            const AltitudeKalmanFilter::state_t kalmanFilterState =_context.altitude_kalman_filter.update_state(altitude_measurement, acceleration_measurement, delta_t);

            const altitude_data_t altitude_data {
                .altitude_meters = kalmanFilterState.altitude,
                .altitude_velocity_meters_per_second = kalmanFilterState.velocity
            };
            _context.altitude_message_queue.SEND_ALTITUDE_DATA(altitude_data);
        }
    }
#else
    while (true) {}
...
*/
[[noreturn]] void AltitudeTask::task_static(void* arg)
{
