...
AltitudeKalmanFilter::state_t AltitudeKalmanFilter::update_state(float altitude_measurement, float acceleration_measurement, float dT)
{
    // predicted state
    _predicted.velocity = _estimated.velocity + (acceleration_measurement - _estimated.bias)*dT;
    ...
    // update estimate
    const float e = altitude_measurement - _predicted.altitude;
...
void AltitudeKalmanFilter::set_velocity(float velocity)
{
