...
#include "rc_adjustments.h" // for rc_adjustment_ranges_t
#include "rc_modes.h" // for rc_modes_activation_condition_array_t
...
public:
    ...
    static constexpr int8_t OK_IS_DEFAULT = 1;

    static constexpr int8_t ERROR_FLASH_FULL = -1;
    static constexpr int8_t  ERROR_NOT_FOUND = -2;
    static constexpr int8_t  ERROR_NOT_WRITTEN = -3;
    static constexpr int8_t  ERROR_INVALID_PROFILE = - 4;

    static constexpr bool READ_WRITE = false;
    static constexpr bool  READ_ONLY = true;
    static constexpr uint8_t MAC_ADDRESS_LEN = 6;

    enum calibration_state_e { NOT_CALIBRATED = 0, CALIBRATED = 1 };
    static constexpr uint8_t DEFAULT_PID_PROFILE = 0;
    static constexpr uint8_t PID_PROFILE_COUNT = 4;
    static constexpr uint8_t DEFAULT_RATE_PROFILE = 0;
    static constexpr uint8_t RATE_PROFILE_COUNT = 4;
...
public:
    ...
    int32_t store_all(const ImuFilters& imu_filters, const FlightController& flight_controller, const MotorMixerBase& motor_mixer, const Cockpit& cockpit, const RcModes& rc_modes);
    ...
    int32_t store_rc_adjustment_ranges(const rc_adjustment_ranges_t& adjustment_ranges);
#endif
    rates_t load_rates(uint8_t rate_profile_index) const;
    int32_t store_rates(const rates_t& rates, uint8_t rate_profile_index);

    pid_constants_uint16_t load_pid(uint8_t pid_index, uint8_t pid_profile_index) const;
    int32_t store_pid(const pid_constants_uint16_t& pid, uint8_t pid_index, uint8_t pid_profile_index);
    int32_t store_pid(const pid_constants_uint16_t& pid, uint8_t pid_index) { return store_pid(pid, pid_index, _current_pid_profile_index); }
...
#if defined(USE_DMAX)
    dmax_config_t load_flight_controller_dmax_config(uint8_t pid_profile_index) const;
    int32_t store_flight_controller_dmax_config(const dmax_config_t& config, uint8_t pid_profile_index);
#endif
