...
tpa_config_t NonVolatileStorage::load_flight_controller_tpa_config(uint8_t pid_profile_index) const
{
    ...
    return DEFAULTS::FLIGHT_CONTROLLER_TPA_CONFIG;
}

int32_t NonVolatileStorage::store_flight_controller_tpa_config(const tpa_config_t& config, uint8_t pid_profile_index)
{
    return store_item(FLIGHT_CONTROLLER_TPA_CONFIG_KEY, pid_profile_index, &config, sizeof(config), &DEFAULTS::FLIGHT_CONTROLLER_TPA_CONFIG);
}
...
#if defined(USE_DMAX)
dmax_config_t NonVolatileStorage::load_flight_controller_dmax_config(uint8_t pid_profile_index) const
{
    {dmax_config_t config {};
    ...
    return DEFAULTS::FLIGHT_CONTROLLER_DMAX_CONFIG;
}

int32_t NonVolatileStorage::store_flight_controller_dmax_config(const dmax_config_t& config, uint8_t pid_profile_index)
{
    return store_item(FLIGHT_CONTROLLER_DMAX_CONFIG_KEY, pid_profile_index, &config, sizeof(config), &DEFAULTS::FLIGHT_CONTROLLER_DMAX_CONFIG);
}
#endif
...
{
    {rc_adjustment_ranges_t adjustment_ranges {};
    if (load_item(RC_ADJUSTMENT_RANGES_KEY, &adjustment_ranges, sizeof(adjustment_ranges))) {
        return adjustment_ranges;
    }}
    return DEFAULTS::RC_ADJUSTMENT_RANGES;
}

int32_t NonVolatileStorage::store_rc_adjustment_ranges(const rc_adjustment_ranges_t& adjustment_ranges)
{
    return store_item(RC_ADJUSTMENT_RANGES_KEY, &adjustment_ranges, sizeof(adjustment_ranges), &DEFAULTS::RC_ADJUSTMENT_RANGES);
}
#endif
...
pid_constants_uint16_t NonVolatileStorage::load_pid(uint8_t pid_index, uint8_t pid_profile_index) const
{
    assert(pid_index <= FlightController::PID_COUNT);
    {pid_constants_uint16_t pid {};
...
int32_t NonVolatileStorage::store_pid(const pid_constants_uint16_t& pid, uint8_t pid_index, uint8_t pid_profile_index)
{
    assert(pid_index <= FlightController::PID_COUNT);
    if (pid_profile_index >= PID_PROFILE_COUNT) {
        return ERROR_INVALID_PROFILE;
...
int32_t NonVolatileStorage::store_all(const ImuFilters& imu_filters, const FlightController& flight_controller, const MotorMixerBase& motor_mixer, const Cockpit& cockpit, const RcModes& rc_modes)
{
    (void)motor_mixer;
#if defined(USE_DYNAMIC_IDLE)
    const dynamic_idle_controller_config_t* dynamicIdleControllerConfig = motor_mixer.get_dynamic_idle_config();
...
#endif
#if defined(USE_RPM_FILTERS)
    const RpmFilters* rpm_filters = imu_filters.get_rpm_filters();
    if (rpm_filters) {
        store_rpm_filters_config(rpm_filters->get_config());
    }
#endif

    store_flight_controller_filters_config(flight_controller.get_filters_config(), _current_pid_profile_index);

    store_flight_controller_flight_mode_config(flight_controller.get_flight_mode_config(), _current_pid_profile_index);

    store_flight_controller_tpa_config(flight_controller.get_tpa_config(), _current_pid_profile_index);

    store_flight_controller_anti_gravity_config(flight_controller.get_anti_gravity_config(), _current_pid_profile_index);

    store_flight_controller_crash_flip_config(flight_controller.get_crash_flip_config());

#if defined(USE_DMAX)
    store_flight_controller_dmax_config(flight_controller.get_dmax_config(), _current_pid_profile_index);
#endif
#if defined(USE_ITERM_RELAX)
    store_flight_controller_iterm_relax_config(flight_controller.get_iterm_relax_config(), _current_pid_profile_index);
#endif
#if defined(USE_YAW_SPIN_RECOVERY)
    store_flight_controller_yaw_spin_recovery_config(flight_controller.get_yaw_spin_recovery_config(), _current_pid_profile_index);
#endif
#if defined(USE_CRASH_RECOVERY)
    store_flight_controller_crash_recovery_config(flight_controller.get_crash_recovery_config(), _current_pid_profile_index);
#endif
#if defined(USE_ALTITUDE_HOLD)
    store_altitude_hold_config(cockpit.get_autopilot().get_altitude_hold_config());
#endif

    const imu_filters_config_t imu_filters_config = imu_filters.get_config();
    store_imu_filters_config(imu_filters_config);

#if defined(USE_DYNAMIC_NOTCH_FILTER)
    store_dynamic_notch_filter_config(imu_filters.get_dynamic_notch_filter_config());
#endif

    store_rates(cockpit.get_rates(), _current_rate_profile_index);
    store_rc_mode_activation_conditions(rc_modes.get_mode_activation_conditions());
#if defined(USE_RC_ADJUSTMENTS)
    store_rc_adjustment_ranges(cockpit.get_rc_adjustments().get_adjustment_ranges());
#endif
...
#if defined(USE_CMS)
void CMSX::save_config_and_notify(cms_context_t& ctx)
{
    ctx.nvs.store_all(ctx.imu_filters, ctx.flight_controller, ctx.motor_mixer, ctx.cockpit, ctx.rc_modes);
}

uint8_t CMSX::get_current_pid_profile_index(cms_context_t& ctx) const
{
    return ctx.nvs.get_current_pid_profile_index();
}

void CMSX::set_current_pid_profile_index(cms_context_t& ctx, uint8_t current_pid_profile_index)
{
    ctx.nvs.set_current_pid_profile_index(current_pid_profile_index);
}

uint8_t CMSX::get_current_rate_profile_index(cms_context_t& ctx) const
{
    return ctx.nvs.get_current_rate_profile_index();
}

void CMSX::set_current_rate_profile_index(cms_context_t& ctx, uint8_t current_rate_profile_index)
{
    ctx.nvs.set_current_rate_profile_index(current_rate_profile_index);
}
#endif
