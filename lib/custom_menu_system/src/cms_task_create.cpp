...
CMS_Task* CMS_Task::create_task(CMS& cms, cms_context_t& context, uint8_t priority, uint32_t core, uint32_t task_interval_microseconds)
{
    task_info_t task_info {};
    return create_task(task_info, cms, context, priority, core, task_interval_microseconds);
}

CMS_Task* CMS_Task::create_task(task_info_t& task_info, CMS& cms, cms_context_t& context, uint8_t priority, uint32_t core, uint32_t task_interval_microseconds)
{
    // Note that task parameters must not be on the stack, since they are used when the task is started, which is after this function returns.
    static CMS_Task cms_task(task_interval_microseconds, cms, context);
