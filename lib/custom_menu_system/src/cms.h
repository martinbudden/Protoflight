...
public:
    const cms_config_t& get_config() const { return _config; }
    void set_config(const cms_config_t& config);
    ...
    uint16_t handle_key_with_repeat(cms_context_t& ctx, cmsx_key_e key, size_t repeat_count);
    bool scan_keys(cms_context_t& ctx, int32_t time_delta, const receiver_controls_pwm_t& controls, bool is_armed);
    void set_extern_key(cmsx_key_e extern_key);
private:
    CMSX _cmsx;
    cms_config_t _config {};
