...
public:
    virtual ~GPS() = default;
    GPS(SerialPort& serial_port, GPS_MessageQueue& gpsMessageQueue) : _serial_port(serial_port), _gpsMessageQueue(gpsMessageQueue) {}
    void init();
...
public:
    void set_config(const gps_config_t& config) { _config = config; }
    const gps_config_t& get_config() const { return _config; }
private:
