...
void FlightController::initializeSetpointFilters(float setpoint_deltaT) // NOLINT(readability-make-member-function-const)
{
    if (_antiGravityConfig.cutoff_hz == 0) {
        _sh.antiGravityThrottleFilter.set_to_passthrough();
    } else {
        _sh.antiGravityThrottleFilter.set_cutoff_frequency(_antiGravityConfig.cutoff_hz, setpoint_deltaT);
    }
    // Feedforward filters
    if (_filtersConfig.rc_smoothing_feedforward_cutoff == 0) {
        for (auto& filter : _sh.setpoint_derivativeFilters) {
            filter.set_to_passthrough();
        }
    } else {
        for (auto& filter : _sh.setpoint_derivativeFilters) {
            filter.set_cutoff_frequency_and_reset(_filtersConfig.rc_smoothing_feedforward_cutoff, setpoint_deltaT);
        }
    }

#if defined(USE_D_MAX)
    for (auto& filter : _sh.dMaxRangeFilters) {
        filter.set_cutoff_frequency(DMAX_RANGE_HZ, setpoint_deltaT);
    }
    for (auto& filter : _sh.dMaxLowpassFilters) {
        filter.set_cutoff_frequency(DMAX_LOWPASS_HZ, setpoint_deltaT);
    }
#endif
#if defined(USE_ITERM_RELAX)
    for (auto& filter : _sh.iTermRelaxFilters) {
        filter.set_cutoff_frequency(_iTermRelaxConfig.iterm_relax_cutoff, setpoint_deltaT);
    }
#endif
}
...
D_MAX is not applied here, since it depends on the gyro value and so needs to be calculated in update_outputs_using_pids()
*/
void FlightController::applyDynamicPID_AdjustmentsOnThrottleChange(float throttle, uint32_t tick_count, Debug& debug)
{
    ...
    if (_rxM.setpointTickCountCounter != 0) {
        ...
        if (_rxM.setpointTickCountCounter == 0) {
            _rxM.setpoint_deltaT = 0.001F * static_cast<float>(_rxM.setpointTickCountSum) / static_cast<float>(rx_t::SETPOINT_TICKCOUNT_COUNTER_START);
            initializeSetpointFilters(_rxM.setpoint_deltaT);
        }
    }

    const float throttleDelta = std::fabs(throttle - _rxM.throttlePrevious);
    _rxM.throttlePrevious = throttle;
    const float deltaT = static_cast<float>((tick_count - _rxM.setpointTickCountPrevious)) * 0.001F;
    _rxM.setpointTickCountPrevious = tick_count;
    ...
    _sh.PIDS[ROLL_RATE_DPS].set_i(_fcC.pidConstants[ROLL_RATE_DPS].ki + iTermAccelerator);
    _sh.PIDS[PITCH_RATE_DPS].set_i(_fcC.pidConstants[PITCH_RATE_DPS].ki + iTermAccelerator);
    ...
    _sh.PIDS[ROLL_RATE_DPS].set_p(_fcC.pidConstants[ROLL_RATE_DPS].kp * PTermBoostRoll * _rxM.TPA);
    ...
    _sh.PIDS[PITCH_RATE_DPS].set_p(_fcC.pidConstants[PITCH_RATE_DPS].kp * PTermBoostPitch * _rxM.TPA);
...
{
    _rxM.TPA = 1.0F;
    _sh.PIDS[ROLL_RATE_DPS].set_i(_fcC.pidConstants[ROLL_RATE_DPS].ki);
    _sh.PIDS[PITCH_RATE_DPS].set_i(_fcC.pidConstants[PITCH_RATE_DPS].ki);
    _sh.PIDS[ROLL_RATE_DPS].set_p(_fcC.pidConstants[ROLL_RATE_DPS].kp);
    _sh.PIDS[PITCH_RATE_DPS].set_p(_fcC.pidConstants[PITCH_RATE_DPS].kp);
}
...
*/
void FlightController::updateSetpoints(const controls_t& controls, failsafe_e failsafe, Debug& debug)
{
    detectCrashOrSpin();
    ...
    if (failsafe == FAILSAFE_ON || _sh.crashDetected || _sh.yawSpinRecovery || _sh.crashFlipModeActive) {
        ...
        applyDynamicPID_AdjustmentsOnThrottleChange(controls.throttleStick, controls.tick_count, debug);
    }
    ...
    if (!_rxM.useAngleMode) {
        _sh.PIDS[ROLL_RATE_DPS].set_setpoint(controls.rollStickDPS);
    }
    if (_rxM.setpoint_deltaT != 0) {
        if (failsafe == FAILSAFE_ON || _sh.crashDetected || _sh.yawSpinRecovery || _sh.crashFlipModeActive) {
            _sh.PIDS[ROLL_RATE_DPS].set_setpoint_derivative(0.0F);
        } else {
            float setpoint_derivative = _sh.PIDS[ROLL_RATE_DPS].get_setpoint_delta() / _rxM.setpoint_deltaT;
            setpoint_derivative = _sh.setpoint_derivativeFilters[ROLL_RATE_DPS].filter(setpoint_derivative);
...
#endif
    _sh.PIDS[ROLL_ANGLE_DEGREES].set_setpoint(controls.rollStickDegrees);
#if defined(USE_SIN_ANGLE_PIDS)
    _sh.PIDS[ROLL_SIN_ANGLE].set_setpoint(sinf(controls.rollStickDegrees * DEGREES_TO_RADIANS));
#endif
    ...
    if (!_rxM.useAngleMode) {
        _sh.PIDS[PITCH_RATE_DPS].set_setpoint(-controls.pitchStickDPS);
    }
    if (_rxM.setpoint_deltaT != 0) {
        if (failsafe == FAILSAFE_ON || _sh.crashDetected || _sh.yawSpinRecovery || _sh.crashFlipModeActive) {
            _sh.PIDS[PITCH_RATE_DPS].set_setpoint_derivative(0.0F);
        } else {
            float setpoint_derivative = _sh.PIDS[PITCH_RATE_DPS].get_setpoint_delta() / _rxM.setpoint_deltaT;
            setpoint_derivative = _sh.setpoint_derivativeFilters[PITCH_RATE_DPS].filter(setpoint_derivative);
...
#endif
    _sh.PIDS[PITCH_ANGLE_DEGREES].set_setpoint(-controls.pitchStickDegrees);
#if defined(USE_SIN_ANGLE_PIDS)
    _sh.PIDS[ROLL_SIN_ANGLE].set_setpoint(sinf(-controls.pitchStickDegrees * DEGREES_TO_RADIANS));
#endif
    ...
    _sh.PIDS[YAW_RATE_DPS].set_setpoint(controls.yawStickDPS);
    ...
    if (_sh.groundMode) {
        ...
        if (_sh.outputThrottle < _takeOffThrottleThreshold) {
            ...
            if (tick_count - _sh.takeOffCountStart > _takeOffTickThreshold) {
...
void FlightController::detectCrashOrSpin()
{
    ...
    if (_sh.yawSpinThresholdDPS !=0.0F && std::fabs(_sh.PIDS[YAW_RATE_DPS].get_previous_measurement()) > _sh.yawSpinThresholdDPS) {
        // yaw spin detected
    ...
    if (std::fabs(pid.get_error_raw_d()) > _crash.DtermThresholdDPSPS
        && std::fabs(pid.get_error_raw_p()) > _crash.gyroThresholdDPS
        && std::fabs(pid.get_setpoint()) < _crash.setpointThresholdDPS) {
