...
void MSP_Protoflight::reboot_fn(msp_context_t& ctx, serial_port_t* serial_port) // NOLINT(readability-make-member-function-const)
{
    (void)ctx;
    (void)serial_port;
...
msp_result_e MSP_Protoflight::set_passthrough_command(msp_context_t& ctx, StreamBufWriter& dst, StreamBufReader& src) // NOLINT(readability-convert-member-functions-to-static)
{
    (void)ctx;
...
msp_result_e MSP_Protoflight::process_write_command(msp_context_t& ctx, int16_t cmd_msp, StreamBufWriter& dst, StreamBufReader& src)
{
    switch (cmd_msp) {
    case MSP_BOXNAMES: {
        const size_t page = (src.bytes_remaining() > 0) ? src.read_u8() : 0;
        ctx.cockpit.serialize_box_reply_box_name(dst, page);
        break;
    }
    case MSP_BOXIDS: {
        const size_t page = (src.bytes_remaining() > 0) ? src.read_u8() : 0;
        ctx.cockpit.serialize_box_reply_permanent_id(dst, page);
        break;
    }
    ...
    case MSP_DATAFLASH_READ: {
        return msp_fc_data_flash_read_command(ctx, dst, src);
#endif
    case MSP_RESET_CONF: {
    ...
    default:
        return process_get_command(ctx, cmd_msp, dst);
    }
    return MSP_RESULT_ACK;
}
