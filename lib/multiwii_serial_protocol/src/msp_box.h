...
public:
    ...
    static const box_t* find_box_by_permanent_id(uint8_t permanent_id);
    ...
    bool get_active_box_id(uint8_t box_id) const;
    void set_active_box_id(uint8_t box_id);
    void reset_active_box_id(uint8_t box_id);
protected:
    std::bitset<BOX_COUNT> _active_box_ids {};
...
//      { .id = BOX_LEDMAX,     .permanent_id = 14, .name = "LEDMAX" }, (removed)
