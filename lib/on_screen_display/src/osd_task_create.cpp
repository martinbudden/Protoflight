...
OSD_Task* OSD_Task::create_task(OSD& osd, osd_context_t& context, uint8_t priority, uint32_t core, uint32_t task_interval_microseconds)
{
    task_info_t task_info {};
    return create_task(task_info, osd, context, priority, core, task_interval_microseconds);
}

OSD_Task* OSD_Task::create_task(task_info_t& task_info, OSD& osd, osd_context_t& context, uint8_t priority, uint32_t core, uint32_t task_interval_microseconds)
{
    // Note that task parameters must not be on the stack, since they are used when the task is started, which is after this function returns.
    static OSD_Task osd_task(task_interval_microseconds, osd, context);
