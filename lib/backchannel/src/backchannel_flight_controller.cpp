...
bool BackchannelFlightController::packet_control(backchannel_context_t& ctx, const CommandPacketControl& packet)
{
    ...
    case CommandPacketControl::MOTORS_SWITCH_OFF:
        ctx.flight_controller.motors_switch_off(ctx.motor_mixer);
        return true;
    case CommandPacketControl::MOTORS_SWITCH_ON:
        ctx.flight_controller.motors_switch_on(ctx.motor_mixer);
        return true;
    case CommandPacketControl::RESET:
        return false; // not implemented
    case CommandPacketControl::SET_MODE:
        ctx.flight_controller.set_control_mode(static_cast<fc_control_mode_e>(packet.value));
        return true;
    ...
    case CommandPacketControl::SET_DEBUG_MODE:
        ctx.debug.set_mode(packet.value);
        return true;
    default:
        // do nothing
        break;
...
bool BackchannelFlightController::packet_set_pid(backchannel_context_t& ctx, const CommandPacketSetPid& packet)
{
    ...
    case CommandPacketSetPid::SET_P:
        ctx.flight_controller.set_pid_p_msp(pid_index, packet.value);
        transmit = true;
        break;
    case CommandPacketSetPid::SET_I:
        ctx.flight_controller.set_pid_i_msp(pid_index, packet.value);
        transmit = true;
        break;
    case CommandPacketSetPid::SET_D:
        ctx.flight_controller.set_pid_d_msp(pid_index, packet.value);
        transmit = true;
        break;
    case CommandPacketSetPid::SET_S:
        ctx.flight_controller.set_pid_s_msp(pid_index, packet.value);
        transmit = true;
        break;
    case CommandPacketSetPid::SET_K:
        ctx.flight_controller.set_pid_k_msp(pid_index, packet.value);
        transmit = true;
        break;
    case CommandPacketSetPid::SET_PD: {
        // Set P and change D to preserve P/D ratio
        ctx.flight_controller.set_pid_pd_msp(pid_index, packet.value);
        transmit = true;
        break;
    }
    ...
    case CommandPacketSetPid::SAVE_PD:
        ...
        ctx.nvs.store_pid(ctx.flight_controller.get_pid_constants(pid_index), pid_index);
        return true;
    case CommandPacketSetPid::RESET_PID:
        ctx.nvs.reset_pid(pid_index);
        return true;
    default:
    ...
    if (transmit) {
        ...
        const size_t len = pack_telemetry_data_pid(
            ...
            _telemetry_id,
            _sequence_number,
            ctx.flight_controller,
            ctx.nvs.get_current_pid_profile_index(),
            ctx.flight_controller.get_control_mode(),
            0.0F,
            0.0F
        );
...
bool BackchannelFlightController::send_packet(backchannel_context_t& ctx, uint8_t sub_command)
{
    ...
    case CommandPacketRequestData::REQUEST_TASK_INTERVAL_DATA: {
        const size_t len = pack_telemetry_data_task_intervals(_transmit_data_buffer_ptr, _telemetry_id, _sequence_number,
            *ctx.ahrs.get_task(),
            *ctx.flight_controller.get_task(),
            ctx.main_task ?  ctx.main_task->get_tick_count_delta() : 0,
    ...
    case CommandPacketRequestData::REQUEST_TASK_INTERVAL_EXTENDED_DATA: {
        ...
        const size_t len = pack_telemetry_data_task_intervals_extended(_transmit_data_buffer_ptr, _telemetry_id, _sequence_number,
            ...
            ctx.main_task ? ctx.main_task->get_tick_count_delta() : 0,
            _backchannel_transceiver.get_tick_count_delta_and_reset(),
            static_cast<uint32_t>(ctx.receiver.get_dropped_packet_count_delta())
        );
        ...
        td->data.ahrsTimeChecksMicroseconds[4] = ctx.flight_controller.get_time_checks_microseconds(0);
        td->data.ahrsTimeChecksMicroseconds[5] = ctx.flight_controller.get_time_checks_microseconds(1);
        td->data.ahrsTimeChecksMicroseconds[6] = ctx.flight_controller.get_time_checks_microseconds(2);
        td->data.ahrsTimeChecksMicroseconds[7] = ctx.flight_controller.get_time_checks_microseconds(3);
        //Serial.printf("tiLen:%d\r\n", len);
        send_data(_transmit_data_buffer_ptr, len);
        return true;
    }
    ...
    case CommandPacketRequestData::REQUEST_AHRS_DATA: {
        ...
        const size_t len = pack_telemetry_data_ahrs(_transmit_data_buffer_ptr, _telemetry_id, _sequence_number, ctx.ahrs, ahrs_data);
        ...
        const Quaternion orientationENU = ahrs_data.orientation;
    ...
    case CommandPacketRequestData::REQUEST_RECEIVER_DATA: {
        const size_t len = pack_telemetry_data_receiver(_transmit_data_buffer_ptr, _telemetry_id, _sequence_number, ctx.receiver);
    ...
    case CommandPacketRequestData::REQUEST_VEHICLE_CONTROLLER_DATA: {
        const size_t len = pack_telemetry_data_fc_quadcopter(_transmit_data_buffer_ptr, _telemetry_id, _sequence_number, ctx.flight_controller, ctx.motor_mixer);
        //Serial.printf("vcLen:%d\r\n", len);
        send_data(_transmit_data_buffer_ptr, len);
        return true;
    }
    case CommandPacketRequestData::REQUEST_DEBUG_DATA: {
        const size_t len = pack_telemetry_data_debug(_transmit_data_buffer_ptr, _telemetry_id, _sequence_number, ctx.debug);
        //Serial.printf("debugLen:%d\r\n", len);
        send_data(_transmit_data_buffer_ptr, len);
        return true;
    }
    case CommandPacketRequestData::REQUEST_MSP_DATA: {
        if (ctx.msp && ctx.msp_context) {
            const size_t len = pack_telemetry_data_msp(_transmit_data_buffer_ptr, _telemetry_id, _sequence_number, *ctx.msp_context, *ctx.msp, sub_command);
