...
*/
size_t pack_telemetry_data_PID(uint8_t* telemetry_data_ptr, uint32_t id, uint32_t sequence_number, const FlightController& flight_controller)
{
...
#endif
        td->spids[ii].setpoint = flight_controller.get_pid_setpoint(static_cast<FlightController::pid_index_e>(ii));
        td->spids[ii].pid = flight_controller.get_pid_constants(static_cast<FlightController::pid_index_e>(ii));
    }
...
*/
size_t pack_telemetry_data_fc_quadcopter(uint8_t* telemetry_data_ptr, uint32_t id, uint32_t sequence_number, const FlightController& flight_controller, const MotorMixerBase& motor_mixer)
{
    ...
    td->flags = motor_mixer.motors_is_on() ? TD_FC_QUADCOPTER::MOTORS_ON_FLAG : 0x00;
    td->flags |= static_cast<uint16_t>(static_cast<uint16_t>(TD_FC_QUADCOPTER::CONTROL_MODE_MASK) & static_cast<uint16_t>(flight_controller.get_control_mode())); //NOLINT(hicpp-signed-bitwise)

    const flight_controller_quadcopter_telemetry_t telemetryData = flight_controller.get_telemetry_data(motor_mixer);
    //!!memcpy(&td->data, &telemetryData, sizeof(flight_controller_quadcopter_telemetry_t));
    //!!td->pidErrors[TD_FC_QUADCOPTER::ROLL_RATE_DPS] = telemetryData.roll_rate_error;
#if (__cplusplus >= 202002L)
...
size_t pack_telemetry_data_msp(uint8_t* telemetry_data_ptr, uint32_t id, uint32_t sequence_number, msp_context_t& ctx, MspBase& msp, int16_t cmd_msp)
{
    ...
    msp.process_write_command(ctx, cmd_msp, sbuf, sbuf_reader);
